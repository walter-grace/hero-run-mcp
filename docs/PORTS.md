...
- **[hero-run-market-maker](https://github.com/walter-grace/hero-run-market-maker)**: a Go market-making bot for $HERO. Its optional `-brain` flag consults Hero Run itself for regime calls (routing modes as model ids, @gateway pinning, strict-JSON `response_format`). The flywheel as a working example.
